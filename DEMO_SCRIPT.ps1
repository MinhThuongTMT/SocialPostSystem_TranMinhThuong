...
$users = @(
    @{username="alice";email="alice@example.com";password="Alice123!@secure";full_name="Alice Johnson"}
    @{username="bob";email="bob@example.com";password="Bob123!@secure";full_name="Bob Wilson"}
    @{username="charlie";email="charlie@example.com";password="Charlie123!@secure";full_name="Charlie Brown"}
)
...
foreach ($user in $users) {
    try {
        ...
        Write-Host "⚠️  $($user.username) - Already exists or error" -ForegroundColor $Warning
    }
}
Write-Host ""
...
# ─────────────────────────────────────────────────────────────────────
Write-Host "3️⃣  LOGIN & GET JWT TOKENS" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info
...
foreach ($user in $users) {
    try {
        ...
        Write-Host "OK Login: $($user.username) - Token: $($loginData.access_token.Substring(0,20))..." -ForegroundColor $Success
    } catch {
        Write-Host "No Login failed: $($user.username)" -ForegroundColor Red
    }
}
Write-Host ""
...
# ─────────────────────────────────────────────────────────────────────
Write-Host "4️⃣  CREATE POSTS (MongoDB)" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info

$postContents = @(
    "Just launched my new FastAPI project! Rocket",
    "MongoDB + Redis integration working perfectly! Database",
    "JWT authentication is so cool! Lock"
)
...
# ─────────────────────────────────────────────────────────────────────
Write-Host "5️⃣  GET POSTS (Redis Cache)" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info
...
# 6. CREATE LIKES
# ─────────────────────────────────────────────────────────────────────
Write-Host "6️⃣  CREATE LIKES (MongoDB + Redis Pub/Sub)" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info
...
if ($postIds.Count -gt 0) {
    try {
        ...
        Write-Host "⚠️  Like creation - $_" -ForegroundColor $Warning
    }
}
Write-Host ""
...
# ─────────────────────────────────────────────────────────────────────
Write-Host "7️⃣  RATE LIMITING (via Redis)" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info
...
$rateLimitReset = $testReq.Headers["x-ratelimit-reset"]
...
Write-Host "   Reset in: $($rateLimitReset - [int][double]::Parse((Get-Date -UFormat %s))) seconds" -ForegroundColor $Info
Write-Host ""

# ─────────────────────────────────────────────────────────────────────
# 8. REDIS VERIFICATION
# ─────────────────────────────────────────────────────────────────────
Write-Host "8️⃣  REDIS KEYS VERIFICATION" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info

$redisKeys = docker exec -it social_post_redis redis-cli KEYS "*"
...
# ─────────────────────────────────────────────────────────────────────
Write-Host "9️⃣  MONGODB DATA VERIFICATION" -ForegroundColor $Info
Write-Host "─────────────────────────────────────────" -ForegroundColor $Info
...
Write-Host "   📍 Other collections: comments, likes, follows, audit_logs" -ForegroundColor $Info
Write-Host ""
...
Write-Host "🏗️  TECHNOLOGY STACK:" -ForegroundColor $Info
...
Write-Host "Ready for employer demo! 🚀" -ForegroundColor Cyan
